...
describe('RetryConditions', () => {
  ...
  describe('onNetworkError', () => {
    it('should return true for network-related errors', () => {
      expect(RetryConditions.onNetworkError(createMockError('ETIMEDOUT', 'Connection timed out'))).toBe(true);
      expect(RetryConditions.onNetworkError(createMockError('ECONNRESET', 'Connection reset by peer'))).toBe(true);
      expect(RetryConditions.onNetworkError(createMockError('', 'Network error occurred'))).toBe(true);
      expect(RetryConditions.onNetworkError(createMockError('', 'Socket timeout'))).toBe(true);
    });

    it('should return false for non-network errors', () => {
      expect(RetryConditions.onNetworkError(createMockError('400', 'Bad request'))).toBe(false);
      expect(RetryConditions.onNetworkError(createMockError('', 'Validation failed'))).toBe(false);
    });
  });

  describe('onServerError', () => {
    it('should return true for server errors', () => {
      expect(RetryConditions.onServerError(createMockError('500', 'Internal Server Error'))).toBe(true);
      expect(RetryConditions.onServerError(createMockError('503', 'Service Unavailable'))).toBe(true);
      expect(RetryConditions.onServerError(createMockError('', 'Internal server error'))).toBe(true);
      expect(RetryConditions.onServerError(createMockError('', 'Gateway timeout'))).toBe(true);
    });

    it('should return false for client errors', () => {
      expect(RetryConditions.onServerError(createMockError('400', 'Bad Request'))).toBe(false);
      expect(RetryConditions.onServerError(createMockError('404', 'Not Found'))).toBe(false);
    });
  });

  describe('onRateLimit', () => {
    it('should return true for rate limit errors', () => {
      expect(RetryConditions.onRateLimit(createMockError('429', 'Too Many Requests'))).toBe(true);
      expect(RetryConditions.onRateLimit(createMockError('RATE_LIMITED', 'Rate limit exceeded'))).toBe(true);
      expect(RetryConditions.onRateLimit(createMockError('', 'Rate limit reached'))).toBe(true);
    });

    it('should return false for non-rate-limit errors', () => {
      expect(RetryConditions.onRateLimit(createMockError('500', 'Internal Error'))).toBe(false);
      expect(RetryConditions.onRateLimit(createMockError('', 'General error'))).toBe(false);
    });
  });

  describe('onTransientError', () => {
    it('should return true for transient errors', () => {
      expect(RetryConditions.onTransientError(createMockError('500', 'Server error'))).toBe(true);
      expect(RetryConditions.onTransientError(createMockError('ETIMEDOUT', 'Timeout'))).toBe(true);
      expect(RetryConditions.onTransientError(createMockError('429', 'Rate limited'))).toBe(true);
    });

    it('should return false for non-transient errors', () => {
      expect(RetryConditions.onTransientError(createMockError('400', 'Bad request'))).toBe(false);
      expect(RetryConditions.onTransientError(createMockError('401', 'Unauthorized'))).toBe(false);
    });
  });
  ...
  describe('unlessClientError', () => {
    it('should return false for client errors', () => {
      expect(RetryConditions.unlessClientError(createMockError('400', 'Bad Request'))).toBe(false);
      expect(RetryConditions.unlessClientError(createMockError('404', 'Not Found'))).toBe(false);
    });

    it('should return true for rate limits and server errors', () => {
      expect(RetryConditions.unlessClientError(createMockError('429', 'Rate Limited'))).toBe(true);
      expect(RetryConditions.unlessClientError(createMockError('500', 'Server Error'))).toBe(true);
      expect(RetryConditions.unlessClientError(createMockError('ETIMEDOUT', 'Timeout'))).toBe(true);
    });
  });
