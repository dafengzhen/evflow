 ...
 * @author dafengzhen
 */
export class LifecycleManager<EM extends EventMap = EventMap, GC extends PlainObject = PlainObject> {
  ...
  private initializeHooks(lifecycle?: EventBusLifecycleHooks<EM, GC>): Required<EventBusLifecycleHooks<EM, GC>> {
    const noop = () => {};
  ...
  private async executeHook(hookName: keyof EventBusLifecycleHooks<EM, GC>, ...args: any[]): Promise<void> {
    ...
    try {
      ...
      console.error(`Lifecycle hook "${String(hookName)}" execution failed:`, error);
    }
  }
  ...
  async beforeHandler<K extends StringKeyOf<EM>, R = unknown>(
    ...
    await this.executeHook('onBeforeHandler', event, context, handler, handlerIndex, totalHandlers);
  }
  ...
  async afterHandler<K extends StringKeyOf<EM>, R = unknown>(
    ...
    await this.executeHook('onAfterHandler', event, context, handler, result, handlerIndex, totalHandlers);
  }
  ...
  async beforeMiddleware<K extends StringKeyOf<EM>, R = unknown>(
    ...
    await this.executeHook('onBeforeMiddleware', event, context, middleware, middlewareIndex, totalMiddlewares);
  }
  ...
  async noHandlers<K extends StringKeyOf<EM>>(event: K, context: EventContext<EM[K], GC>): Promise<void> {
