...
> implements IEventBus<EM, GC>
{
	...
	use<K extends StringKeyOf<EM>, R = unknown>(
		...
		const targetMap = this.isPatternKey(eventStr)
	...
	useGlobalMiddleware<R = unknown>(
		...
		const idx = this.globalMiddlewares.findIndex(
			(w) => w.middleware === middleware,
		);
		if (idx === -1) {
	...
	private async executeWithGlobalMiddlewares<R = unknown>(
		...
		const next = async (): Promise<void> => {
			...
			if (currentIndex < applicableMiddlewares.length) {
				return applicableMiddlewares[currentIndex].middleware(
					context,
					next,
					info,
				);
	...
	private isPatternKey(key: string): boolean {
		return !!(
			this.patternOptions.wildcard && key.includes(this.patternOptions.wildcard)
		);
	}
	...
	private isPatternMatch(eventName: string, pattern: string): boolean {
		...
		if (
			matchMultiple &&
			patternParts.some((p) => wildcard && p === wildcard + wildcard)
		) {
