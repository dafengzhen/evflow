...
> implements IEventBus<EM, GC>
{
	private readonly handlers = new Map<
		string,
		Array<HandlerWrapper<EM, any, any, GC>>
	>();
	...
	private readonly middlewares = new Map<
		string,
		Array<MiddlewareWrapper<EM, any, any, GC>>
	>();

	private readonly patternMiddlewares = new Map<
		string,
		Array<MiddlewareWrapper<EM, any, any, GC>>
	>();

	private readonly globalMiddlewares: Array<
		MiddlewareWrapper<EM, StringKeyOf<EM>, any, GC>
	> = [];
	...
	destroy(): void {
		void this.lifecycleManager
			.destroy()
			.catch((err) =>
				console.error('Error during lifecycle manager destruction:', err),
			);
	...
	async emit<K extends StringKeyOf<EM>, R = unknown>(
		...
		try {
			...
			await this.lifecycleManager.beforeEmit(
				eventName as K,
				enhancedContext,
				emitOptions,
			);
			...
			if (allHandlers.length === 0) {
				return await this.handleNoHandlers(
					eventName as K,
					enhancedContext,
					emitOptions,
				);
			...
			await this.lifecycleManager.afterEmit(
				eventName as K,
				enhancedContext,
				results,
				emitOptions,
			...
			return await this.handleEmitError(
				eventName as K,
				context,
				error,
				emitOptions,
	...
	use<K extends StringKeyOf<EM>, R = unknown>(
		...
		const wrapper: MiddlewareWrapper<EM, K, R, GC> = {
			...
			throwOnEventError:
				options?.throwOnEventError ?? DEFAULT_THROW_ON_EVENT_ERROR,
		};

		const eventStr = String(event);
		const map = this.isPatternKey(eventStr)
	...
	useGlobalMiddleware<R = unknown>(
		...
		if (!this.globalMiddlewares.some((w) => w.middleware === middleware)) {
			this.globalMiddlewares.push(wrapper);
			this.globalMiddlewares.sort(sortByPriorityDesc);
		}

		return () => {
			const i = this.globalMiddlewares.findIndex(
				(w) => w.middleware === middleware,
			);
			if (i !== -1) {
	...
	private removeFromMapByIdentity<T>(
		map: Map<string, T[]>,
		key: string,
		identity: (w: T) => boolean,
	): void {
	...
	private unmatchAllHandlers(handler: EventHandler<EM, any, any, GC>): void {
		for (const [pattern, handlers] of Array.from(
			this.patternHandlers.entries(),
		)) {
	...
	private cleanupOnceHandlers<K extends StringKeyOf<EM>>(
		...
		const once = handlers.filter((h) => h.once);
		if (once.length === 0) {
			return;
		}
		for (const wrapper of once) {
			...
			const executeMiddlewareChain = async (index: number): Promise<R> => {
				if (index < middlewares.length) {
					...
					try {
						...
						const eventError = this.normalizeError(err);
						info.lifecycle!.phase = LifecyclePhase.AFTER_MIDDLEWARE;
						info.lifecycle!.endTime = Date.now();
						await this.lifecycleManager.afterMiddleware(
							context.meta?.eventName as any,
	...
	private createHandlerExecutor<R>(
		...
		return async (
			handlerWrapper: HandlerWrapper<EM, any, R, GC>,
			handlerIndex: number,
		): Promise<void> => {
			if (stopFlag?.value) {
				return;
			...
			try {
				...
				const failedResult = this.createFailedResult<R>(err, options.traceId);
				...
				await this.handleEventResultError(
					failedResult,
					eventMiddlewares,
					context,
				);
	...
	private async processEvent<K extends StringKeyOf<EM>, R = unknown>(
		...
		const eventMiddlewares = this.getFilteredMiddlewares(
			String(event),
			context,
		);
		...
		try {
				...
				async () => {
					try {
						await this.executeHandlers(
							allHandlers,
							async (handler, index) => {
								await this.lifecycleManager.beforeHandler(
									event,
									context,
									handler.handler,
									index,
								...
								executionInfo.inProgress =
									results.length < executionInfo.handlerCount;
							},
							effectiveParallel,
							maxConcurrency,
							stopFlag,
			...
			const eventError = this.normalizeError(err);
			await this.lifecycleManager.onError(
				event,
				context,
				eventError,
				LifecyclePhase.ERROR_HANDLING,
			);
			results.push(this.createFailedResult<R>(err, traceId));
		}
	...
	private async executeParallel<K extends StringKeyOf<EM>, R>(
		...
		for (let i = 0; i < handlers.length; i++) {
			...
			const p = executeHandler(handlers[i], i);
	...
	private async executeWithGlobalMiddlewares<R = unknown>(
		...
		const next = async (): Promise<void> => {
			idx++;
			if (idx < applicableMiddlewares.length) {
				return applicableMiddlewares[idx].middleware(context, next, info);
			}
		...
		return new Promise<T>((resolve, reject) => {
			...
			const timer = setTimeout(async () => {
				...
				if (event && context && phase) {
					...
					try {
						await this.lifecycleManager.onTimeout(
							event as any,
							context,
							timeout,
							phase,
	...
	private getAllHandlersForEvent<K extends StringKeyOf<EM>>(
		event: string,
	): Array<HandlerWrapper<EM, K, any, GC>> {
		const exactHandlers = (this.handlers.get(event) ?? []) as Array<
			HandlerWrapper<EM, K, any, GC>
		>;
		const patternHandlers = this.getMatchingFromMap(
			this.patternHandlers,
			event,
		);
	...
	private getFilteredMiddlewares<K extends StringKeyOf<EM>>(
		...
		const exact = (this.middlewares.get(event) ?? []) as Array<
		...
		return this.deduplicateMiddlewares(combined).filter(
			(mw) => !mw.filter || mw.filter(context),
		);
	}

	private getMatchingFromMap<T extends { priority?: number }>(
		map: Map<string, T[]>,
		eventName: string,
	): T[] {
		const result: T[] = [];
	...
	private isPatternKey(key: string): boolean {
		return Boolean(
			this.patternOptions.wildcard &&
				key.includes(this.patternOptions.wildcard),
		);
	}
	...
	private isPatternMatch(eventName: string, pattern: string): boolean {
		...
		if (matchMultiple && pattern === wildcard + wildcard) {
			return true;
		}
	...
	private matchSimpleSegments(
		...
		for (let i = 0; i < patternParts.length; i++) {
			...
			if (p === wildcard) {
				if (COMMON_WILDCARD_CHARS.has(e)) {
					return false;
				}
				continue;
			}
	...
	private matchWithDoubleWildcard(
		...
		const { allowZeroLengthDoubleWildcard = false, wildcard } =
			this.patternOptions;
		if (!wildcard) {
			return false;
		}
		...
		if (currentPattern === doubleWildcard) {
			...
			const startIndex = allowZeroLengthDoubleWildcard
				? eventIndex
				: eventIndex + 1;
			for (let i = startIndex; i <= eventParts.length; i++) {
				if (
					this.matchWithDoubleWildcard(
						eventParts,
						patternParts,
						i,
						patternIndex + 1,
		...
		return this.matchWithDoubleWildcard(
			eventParts,
			patternParts,
			eventIndex + 1,
			patternIndex + 1,
	...
	private async handleNoHandlers<K extends StringKeyOf<EM>, R = unknown>(
		...
		if (!emitOptions?.ignoreNoHandlersWarning) {
			console.trace(
				`[EventBus] No handlers found for event "${String(event)}".`,
				'Context:',
				context,
			);
		}
	...
	private enhanceContext<K extends StringKeyOf<EM>>(
		...
		return {
			...context,
			meta: { eventName: event, ...context?.meta },
	...
	private async safeUninstall(plugin: EventBusPlugin<EM, GC>): Promise<void> {
		try {
			const res = plugin.uninstall?.(this);
	...
	private initialize(options?: EventBusOptions<EM, GC>): void {
		if (options?.globalMiddlewares?.length) {
