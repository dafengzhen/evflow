...
> implements EventBusPlugin<EM, GC>
{
	...
	constructor(options: PerfMonitorOptions = {}) {
		this.includeFailed =
			options.includeFailedInStats ?? DEFAULTS.INCLUDE_FAILED;
	...
	getPercentile(eventName: string, percentile: number): null | number {
		...
		const index = Math.min(
			sorted.length - 1,
			Math.trunc((percentile / 100) * sorted.length),
		);
		return sorted[index];
	}

	install(bus: IEventBus<EM, GC>): void {
		this.removeMiddleware = bus.useGlobalMiddleware(this.monitor(), {
			priority: 0,
	...
	private record(
		...
		if (succeeded || this.includeFailed) {
			...
			stat.minMs =
				stat.minMs === null ? duration : Math.min(stat.minMs, duration);
			stat.maxMs =
				stat.maxMs === null ? duration : Math.max(stat.maxMs, duration);
