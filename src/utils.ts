...
export const RetryStrategies = {
  exponential(baseDelay: number, maxDelay?: number): (attempt: number) => number {
  ...
  jitter(baseDelay: number, jitterFactor: number = 0.5): (attempt: number) => number {
    return (attempt: number) => {
      const delay = baseDelay * Math.pow(2, attempt - 1);
  ...
  linear(baseDelay: number, increment: number, maxDelay?: number): (attempt: number) => number {
    return (attempt: number) => {
...
export const RetryConditions = {
   ...
   */
  and(...conditions: ((error: EventError) => boolean)[]): (error: EventError) => boolean {
   ...
   */
  not(condition: (error: EventError) => boolean): (error: EventError) => boolean {
    return (error: EventError) => {
  ...
  onDatabaseError: (error: EventError): boolean => {
    ...
    return databaseErrorPatterns.some((pattern) => message.includes(pattern) || code.includes(pattern));
  },
  ...
  onNetworkError(error: EventError): boolean {
    ...
    return networkErrorPatterns.some((pattern) => message.includes(pattern) || code.includes(pattern));
  },
  ...
  onServerError(error: EventError): boolean {
    ...
    const serverErrorPatterns = ['internal server error', 'service unavailable', 'bad gateway', 'gateway timeout'];
  ...
  onServiceUnavailable: (error: EventError): boolean => {
    const serviceErrorPatterns = ['service unavailable', 'third party', 'external service', 'upstream', 'provider'];

    const message = error.message.toLowerCase();
    return RetryConditions.onServerError(error) || serviceErrorPatterns.some((pattern) => message.includes(pattern));
  },

  /**
   * Retry on specific HTTP status codes
   */
  onStatusCodes(...statusCodes: (number | string)[]): (error: EventError) => boolean {
    return (error: EventError) => {
   ...
   */
  or(...conditions: ((error: EventError) => boolean)[]): (error: EventError) => boolean {
    return (error: EventError) => {
  ...
  unlessClientError: (error: EventError): boolean => {
    ...
    const clientErrorPatterns = ['bad request', 'unauthorized', 'forbidden', 'not found', 'method not allowed'];
